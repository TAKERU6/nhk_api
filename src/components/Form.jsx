...
class Form extends Component {
  ...
  onSubmit = (e) => {
    ...
    this.props.getTitle(dataArea, dataDate, dataGenre);
  ...
  render() {
    ...
    const getGenre = (genre) => {
      if (genre === "0000") return "ニュース";
      if (genre === "0300") return "ドラマ";
      if (genre === "0502") return "バラエティ";
    };

    const getArea = (area) => {
      if (area === "130") return "東京";
      if (area === "270") return "大阪";
      if (area === "400") return "福岡";
    };

    const isResult = !!Object.keys(resultTitle).length;
    ...
    return (
      <>
        ...
        {isResult && (
          <div>
            ジャンル:{getGenre(resultTitle.genre)}
            <br />
            エリア:{getArea(resultTitle.area)}
            <br />
            日時:{resultTitle.date}
          </div>
        )}
...
const mapStateToProps = (state) => ({ resultTitle: state.resultTitle });

const mapDispatchToProps = (dispatch) => {
  return {
    getUrls: (dataArea, dataDate, dataGenre) =>
      dispatch(getUrls(dataArea, dataDate, dataGenre)),
    getTitle: (dataArea, dataDate, dataGenre) =>
      dispatch(getTitle(dataArea, dataDate, dataGenre)),
  };
};
